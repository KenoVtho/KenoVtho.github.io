function CGame(oData){
    ...
    this._init = function(){
        ...
        _oInterface.refreshMoney(_iCurCredit.toFixed(2));//tron
    };
	...
	this.updateCredit = function(Credit, updateText){
		_iCurCredit = Credit;
        if(updateText)_oInterface.refreshMoney(_iCurCredit.toFixed(2));
	}
    //tron
    this.play = function(){
        if(_iTotalNum < 2 || _iTotalNum > 10)return;
		if(window.wallet_requesting)return;
		window.wallet_requesting = true;
		//
		var _aListForServer = [];
		for(var i = 0; i < _aListSelected.length; i++)_aListForServer[i] = _aListSelected[i]+1;
		userBetSend(_aListForServer, _iCurBet, this.serverReturns);
	}
	//tron
	this.serverReturns = function(result, hits){
		if(!window.wallet_requesting)return;
		window.wallet_requesting = false;
		//
		if(typeof result == "object"){
        	this.play2(true, result, hits);
		}
	}
	//tron
	this.play2 = function(server, result, hits){
        //Detect max prize to win
        this._clearAllExtracted();
        this.smartBlockGUI(false);
        _oInterface.refreshMoney(_iCurCredit.toFixed(2));
        
		if(server){
			_aCombination = result;
			_iHitsNumber = hits;
    ...
    //START_CREDIT = oData.start_player_money;
